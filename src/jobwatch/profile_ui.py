...
from jobwatch.profile import MAX_PROFILE_FIELD_LENGTH, ProfileDetails
...
def render_profile(
    ...
    *,
    ...
    intro = (
        "Vos catégories sont prêtes. Cette dernière étape est facultative : "
        "elle aide Jobwatch à écrire des lettres qui vous ressemblent."
        if welcome
        else "Ajoutez les éléments que vous voulez retrouver dans vos prochains brouillons."
    )
...
form {{ display:grid; gap:13px }} .field {{ display:grid; gap:7px; padding:17px;
...
@media (max-width:520px) {{ .top {{ align-items:flex-start }} .actions {{ justify-content:stretch }}
  ...
  <p class="eyebrow">Votre profil</p><h1>Des lettres qui parlent vraiment de vous.</h1>
  <p class="intro">{html.escape(intro)}</p>
  <p class="note">Tous les champs sont facultatifs. Jobwatch utilise seulement ce que vous écrivez
    ici et votre CV. Il ne doit jamais inventer une expérience, un projet ou une contrainte.</p>
