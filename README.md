...
| `sources` | Les job boards à surveiller. `france_travail` nécessite `client_id`, `client_secret`, `keywords` et éventuellement `department`. `smartrecruiters` prend une liste de slugs de sociétés et des listes facultatives de codes pays `countries` et de régions `regions`, avec `interval_days` (1 ou 4) par défaut du bloc et `company_intervals` pour un calendrier par société. `linkedin` prend une liste de couples `keywords`/`location` et une fenêtre `hours`. Avec `from_profile: true`, les catégories confirmées remplacent ces requêtes ; sans profil confirmé, LinkedIn reste inactif. Ce mode autorise `searches: []`. Voir le [guide du profil](ops/README.md#créer-une-instance) pour les localisations et le télétravail. `wttj` prend ses requêtes, pays, villes internationales, fenêtre `hours` et les identifiants publics de l'index Algolia utilisé par le site. `talentsoft` prend une liste de `sites` (`url` du portail, `name` employeur affiché) et lit le flux RSS public de chacun, limité aux 20 offres les plus récentes : un `interval_days` de 1 est conseillé. `workday` prend la même liste de `sites` pour les portails `*.myworkdayjobs.com` et interroge leur API cxs publique (20 dernières offres, sans filtre de date serveur). `playwright` rend chaque page de liste avec Chromium : chaque site déclare `url`, `name` et `link_pattern` (une regex reconnaissant les liens d'offres, le titre venant du texte de l'ancre) ; zéro lien trouvé compte comme un échec. À réserver aux sites qui bloquent le HTTP simple, car elle lance un vrai navigateur. Chaque source, et chaque société SmartRecruiters ou site Talentsoft, avance sur son propre calendrier `interval_days` (1 ou 4 jours) : un échec réseau ne fait pas avancer son dernier succès, et la fenêtre de recherche couvre tout le retard accumulé. |
...
| `research` | Recherche web large facultative après les collecteurs directs : runner `codex`, `opencode` ou `openrouter`. Avec `openrouter`, appel HTTP direct avec la clé `api_key`, le modèle `model` et, si besoin, le fournisseur imposé `provider` (par exemple `fireworks`, sans repli) ; la recherche web passe par le plugin `web` d'OpenRouter. Fenêtre `recency_days`, plafond `max_results` (appliqué après validation et déduplication) et instructions de profil. Les offres récentes encore sans fit rejoignent aussi les candidats à évaluer. Codex ignore la configuration utilisateur et désactive les outils locaux. OpenCode refuse ses outils nommément et autorise seulement `websearch` et `webfetch` pour `research`. |
| `enrich` | Configuration de `jw enrich` : `runner` (`opencode`, défaut, `codex`, `openrouter` ou `pi`), le binaire correspondant (`opencode_bin`/`codex_bin`/`pi_bin`), `api_key` (requis avec `openrouter`, appel HTTP direct sans plugin web), `model` (ex. `opencode/deepseek-v4-flash-free`, `deepseek/deepseek-v4.1-flash`, `gpt-5.6-luna` ou `openai-codex/gpt-5.6-luna` avec Pi), `provider` optionnel pour imposer un fournisseur OpenRouter comme `fireworks` sans repli, `variant` optionnel (effort de raisonnement) et `concurrency` (appels LLM simultanés, défaut 4 ; les fetchs web restent séquentiels). Codex et OpenCode sont exécutés sans outils. Pi est exécuté sans outils, extensions, skills, fichiers de contexte ni session persistante. |
